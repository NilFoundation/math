...
{
    template <typename T>
    static BOOST_GPU_ENABLED BOOST_CXX14_CONSTEXPR T result(T base){ return base; }
};
...
struct power_if_positive
{
    ...
    static BOOST_GPU_ENABLED BOOST_CXX14_CONSTEXPR T result(T base, const Policy&)
...
{
    template <typename T, class Policy>
    static BOOST_GPU_ENABLED BOOST_CXX14_CONSTEXPR T result(T base, const Policy& policy)
    {
...
BOOST_GPU_ENABLED BOOST_CXX14_CONSTEXPR inline typename tools::promote_args<T>::type pow(T base, const Policy& policy)
{ 
   return detail::select_power_if_positive<N>::type::result(static_cast<typename tools::promote_args<T>::type>(base), policy);
}

template <int N, typename T>
BOOST_GPU_ENABLED BOOST_CXX14_CONSTEXPR inline typename tools::promote_args<T>::type pow(T base)
